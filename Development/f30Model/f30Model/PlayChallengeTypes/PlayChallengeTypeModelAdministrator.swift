//
//  PlayChallengeTypeModelAdministrator.swift
//  f30Model
//
...
public class PlayChallengeTypeModelAdministrator: ModelAdministratorBase {
	...
	public func toWrappers() -> [PlayChallengeTypeWrapper] {
		
		var result:             [PlayChallengeTypeWrapper] = [PlayChallengeTypeWrapper]()
		
		if let collection = self.collection {
			
			let collection:     PlayChallengeTypeCollection = collection as! PlayChallengeTypeCollection
			...
			for item in collection.items! {
				...
				if (item.status != .deleted && item.status != .obsolete) {
					...
					// Get item wrapper
					let wrapper: PlayChallengeTypeWrapper = (item as! PlayChallengeType).toWrapper()
	...
	public override func newCollection() -> ProtocolModelItemCollection? {
		...
		return PlayChallengeTypeCollection(modelAdministrator: self,
